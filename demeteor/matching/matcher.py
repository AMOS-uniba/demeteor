...
from demetria.correctors import KernelSmoother, kernels
from demeteor.photometry import Calibration, LogCalibration
from demeteor.sensor import (SensorData, altaz_to_disk, disk_to_altaz, numpy_to_disk,
                             proj_to_disk)
...
class Matcher:
    ...
    def load_catalogue(self, filename: Path = None):
        ...
        Everything an all-sky camera can see, to visual magnitude 6, so a caller has stars to match
    ...
    # Five methods stayed in vasco, and they are the ones that were never about matching:
    # _grid/position_grid/magnitude_grid sample a smoother on a lattice for a quiver plot, and
    # build_stars_table/build_catalogue_table shape a DotMap for a Qt table widget. A library hands
    # you the field and the numbers; choosing a lattice to draw them on, or a column order to show
    # them in, belongs to whatever is doing the drawing. See vasco/plots/grid.py and vasco/tables.py.
